...
const NewOrder = () => {
  ...
  const [selectedKebab, setSelectedKebab] = useState(KEBAB_TYPES[0]);
  ...
  const unitPrice = selectedKebab?.basePrice || 0;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // 🔴 TEKSHIRUV: Mijoz ham, joy ham, tavsif ham yo'q bo'lsa, davom etmaydi
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center justify-between mb-3 bg-gray-50 dark:bg-gray-700 rounded-xl px-2 py-2">
              <button
                onClick={() => adjustQty(-1)}
                className="w-12 h-12 flex items-center justify-center rounded-xl bg-white dark:bg-gray-600 shadow text-2xl text-navy-700 dark:text-white font-bold active:scale-90"
              >
                −
              </button>
              <input
                type="number"
                value={customQtyStr || effectiveQty}
                onChange={(e) =>
                  setCustomQtyStr(e.target.value.replace(/\D/g, ""))
                }
            ...
            <div className="grid grid-cols-5 gap-2">
              {[1, 2, 3, 4, 5].map((n) => {
                const isActive = !customQtyStr && qty === n;
                return (
                  <button
                    key={n}
                    onClick={() => handleQtyPreset(n)}
                    className={`h-11 rounded-xl font-bold text-base active:scale-95 ${isActive ? "bg-primary-500 text-white shadow-md" : "bg-gray-100 dark:bg-gray-700 text-navy-700 dark:text-gray-300"}`}
                  >
                    +{n}
                  </button>
                );
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {KEBAB_TYPES.map((k) => {
                const isSel = selectedKebab?.id === k.id;
                return (
                  <button
                    key={k.id}
                    onClick={() => setSelectedKebab(k)}
                    className={`flex flex-col items-center py-4 px-2 rounded-2xl border-2 active:scale-95 ${isSel ? "border-primary-500 bg-primary-50 dark:bg-primary-950" : "border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800"}`}
                  >
                    <img
                      src={KebabSvg}
                      color={isSel ? "#f97316" : "#2c3260"}
                      size={60}
                    />
                    <span
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-4">
          ...
          <div className="space-y-2.5">
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-500">Mijoz/Joy</span>
              <span className="text-sm font-semibold text-navy-900 dark:text-white text-right max-w-[60%]">
                {orderIdentifier || "—"}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-500">Shashlik turi</span>
            ...
            <div className="border-t border-gray-200 dark:border-gray-600 pt-2.5 flex justify-between items-baseline">
